 ...
 *
 *Basic idea: compute the distance from each building to all the spots, use two 2D array to store the total distance of all the buildings it connects to and how many buildings it connects to accordingly.

When we find a building cannot connect to all the other buildings, we will not find a solution so return -1 right away
 */
/**
 * @author girish_lalwani
 *
        ...
        while(!queue.isEmpty()){
            ...
            for(int i = 0; i < 4; ++i){
                ...
                if(grid[x][y] == 0 && !flag[x][y]){
                    ...
                    // As in above example BFS method will called three times so each zero which reachable from all three buildings 
                    // should have value three as flag array that is visited array will be initialized every time BFS is called
                    // and count array is global;
                    ++count[x][y];
                }
            }
        }
